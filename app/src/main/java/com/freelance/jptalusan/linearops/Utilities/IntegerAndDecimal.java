...
/**
 * Created by jptalusan on 8/6/17.
 */
...
public class IntegerAndDecimal {
    ...
    public ArrayList<Double> validDecimals = new ArrayList<>(Arrays.asList(
...
//        Log.d(TAG, "number: " + divisor + "/" + dividend);
    ...
    public IntegerAndDecimal() {
    }

    boolean isThisNumberValid(double number) {
        long iPart = (long) number;
        double fpart = number - iPart;
        fpart = Math.round((fpart) * 100000.0) / 100000.0;
        System.out.println("w: " + iPart + ", f:" + Math.abs(fpart));
        return validDecimals.contains(Math.abs(fpart)) || (fpart == 0.0 && iPart != 0);
    }
